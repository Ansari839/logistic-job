...
export default function DashboardPage() {
    const { user } = useAuth();
    const accentColor = 'blue'; // Default to blue as animal feed references are removed
            ...
            <div className={`mb-10 relative overflow-hidden p-8 rounded-3xl bg-gradient-to-br from-blue-600/20 to-indigo-600/20 border border-blue-500/20 shadow-xl shadow-blue-500/5`}>
                ...
                <div className="relative z-10">
                    ...
                    <p className="text-subtext max-w-lg leading-relaxed">
                        Your logistics operation at <span className={`text-blue-600 dark:text-blue-400 font-bold`}>{user?.branch || 'Head Office'}</span> is performing <span className="text-emerald-600 dark:text-emerald-400 font-bold">12% better</span> today.
                    </p>
                        ...
                        {[1, 2, 3].map((i) => (
                            <div key={i} className="flex items-start space-x-6 group">
                                <div className="relative mt-1">
                                    <div className={`w-10 h-10 rounded-2xl bg-primary/5 flex items-center justify-center text-blue-600 dark:text-blue-400 group-hover:scale-110 transition-transform border border-border`}>
                                        <CheckCircle size={18} />
                                    </div>
