        ...
        <DashboardLayout>
            <div className="max-w-7xl mx-auto space-y-8">
                ...
                <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
                    <div>
                        <h1 className="text-4xl font-black text-slate-900 dark:text-white tracking-tighter uppercase italic">Journal Vouchers</h1>
                        <p className="text-subtext text-sm font-bold uppercase tracking-[0.2em] mt-1">Manual Adjustments & Provisions</p>
                    </div>
                    <button
                ...
                <div className="grid gap-6">
                    {loading ? (
                        ...
                        <div className="glass-panel p-20 text-center">
                            ...
                            <h3 className="text-xl font-black text-slate-900 dark:text-white">No Journal Vouchers Found</h3>
                            <p className="text-subtext text-sm mt-1">Create your first manual entry to start adjustments.</p>
                        </div>
                    ) : (
                        transactions.map((tx) => (
                            <div key={tx.id} className="glass-card shadow-sm overflow-hidden group">
                                <div className="p-6 border-b border-border flex items-center justify-between bg-primary/5">
                                    <div className="flex items-center gap-6">
                                        <div>
                                            <p className="text-[10px] text-subtext font-black uppercase tracking-widest mb-1">Reference</p>
                                            <h4 className="text-lg font-black text-slate-900 dark:text-white">{tx.reference}</h4>
                                        </div>
                                        <div className="w-px h-8 bg-border" />
                                        <div>
                                            <p className="text-[10px] text-subtext font-black uppercase tracking-widest mb-1">Entry Date</p>
                                            <p className="text-sm font-bold text-slate-700 dark:text-slate-300">{new Date(tx.date).toLocaleDateString()}</p>
                                        </div>
                                    </div>
                                    ...
                                    <table className="w-full text-left border-collapse">
                                        <thead>
                                            <tr className="bg-primary/5">
                                                <th className="px-8 py-4 text-[10px] font-black text-subtext uppercase tracking-widest">Account</th>
                                                <th className="px-8 py-4 text-[10px] font-black text-subtext uppercase tracking-widest">Description</th>
                                                <th className="px-8 py-4 text-[10px] font-black text-subtext uppercase tracking-widest text-right">Debit</th>
                                                <th className="px-8 py-4 text-[10px] font-black text-subtext uppercase tracking-widest text-right">Credit</th>
                                            </tr>
                                        </thead>
                                        <tbody className="divide-y divide-border">
                                            {tx.entries.map((entry) => (
                                                <tr key={entry.id}>
                                                    ...
                                                    <td className="px-8 py-4 text-xs font-medium text-subtext">{entry.description || '-'}</td>
                                                    <td className="px-8 py-4 text-right font-black text-sm text-slate-900 dark:text-white">{entry.debit > 0 ? entry.debit.toLocaleString() : '-'}</td>
                                                    <td className="px-8 py-4 text-right font-black text-sm text-slate-900 dark:text-white">{entry.credit > 0 ? entry.credit.toLocaleString() : '-'}</td>
                                                </tr>
                                            ))}
                                        </tbody>
                                    </table>
                        ...
                        <div className="relative glass-panel w-full max-w-5xl rounded-[3rem] shadow-2xl overflow-hidden animate-in zoom-in duration-300">
                            <div className="p-8 border-b border-border bg-indigo-600 text-white flex justify-between items-center">
                                <div>
                                    ...
                                    <p className="text-indigo-100 text-xs font-bold uppercase tracking-widest">Double-Entry Accuracy Protocol</p>
                                </div>
                                <div className="text-right">
                            ...
                            <form onSubmit={handleSubmit} className="flex flex-col h-[70vh]">
                                <div className="p-8 grid grid-cols-2 md:grid-cols-3 gap-6 border-b border-border">
                                    <div className="space-y-2">
                                        <label className="text-[10px] font-black text-subtext uppercase tracking-widest ml-1">JV Reference</label>
                                        <input
                                            required
                                            className="glass-input w-full rounded-2xl px-5 py-4 text-slate-900 dark:text-white font-mono focus:outline-none focus:ring-2 focus:ring-indigo-500"
                                    ...
                                    <div className="space-y-2">
                                        ...
                                        <input
                                            type="date"
                                            required
                                            className="glass-input w-full rounded-2xl px-5 py-4 text-slate-900 dark:text-white font-bold focus:outline-none focus:ring-2 focus:ring-indigo-500"
                                            value={form.date}
                                            onChange={e => setForm({ ...form, date: e.target.value })}
                                        />
                                    </div>
                                    <div className="space-y-2 col-span-2 md:col-span-1">
                                        <label className="text-[10px] font-black text-subtext uppercase tracking-widest ml-1">Memo / Description</label>
                                        <input
                                            className="glass-input w-full rounded-2xl px-5 py-4 text-slate-900 dark:text-white font-medium focus:outline-none focus:ring-2 focus:ring-indigo-500"
                                ...
                                <div className="flex-1 overflow-y-auto p-8 space-y-4">
                                    {form.entries.map((entry, index) => (
                                        <div key={index} className="grid grid-cols-12 gap-4 items-center animate-in slide-in-from-left-2 duration-200">
                                            <div className="col-span-4">
                                                <select
                                                    className="glass-input w-full rounded-xl px-4 py-3 text-xs font-bold text-slate-900 dark:text-white focus:outline-none focus:ring-2 focus:ring-indigo-500"
                                            ...
                                            <div className="col-span-3">
                                                <input
                                                    className="glass-input w-full rounded-xl px-4 py-3 text-xs font-medium text-slate-900 dark:text-white focus:outline-none focus:ring-2 focus:ring-indigo-500"
                                            ...
                                            <div className="col-span-2">
                                                <input
                                                    type="number"
                                                    className="glass-input w-full rounded-xl px-4 py-3 text-xs font-black text-slate-900 dark:text-white text-right focus:outline-none focus:ring-2 focus:ring-indigo-500"
                                                    placeholder="Debit"
                                                    value={entry.debit || ''}
                                    ...
                                    <button
                                        type="button"
                                        onClick={addEntry}
                                        className="flex items-center gap-2 text-indigo-600 dark:text-indigo-400 font-black uppercase tracking-widest text-[10px] px-4 py-2 rounded-xl hover:bg-indigo-500/10 transition-all border border-indigo-500/20"
                                    >
                                        <Plus size={14} />
                                ...
                                <div className="p-8 border-t border-border flex gap-4">
                                    ...
                                    <button
                                        type="submit"
                                        disabled={!isBalanced}
                                        className={`flex-1 ${isBalanced ? 'bg-indigo-600 shadow-indigo-600/20' : 'bg-primary/10 text-subtext cursor-not-allowed'} text-white font-black py-5 rounded-3xl text-sm uppercase tracking-widest shadow-xl transition-all active:scale-95`}
                                    >
                                        Authorize & Post JV
                                    </button>
                                </div>
